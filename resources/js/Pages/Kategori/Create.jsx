...
export default function KategoriCreate() {
    const { data, setData, post, processing, errors } = useForm({
        nama: '', deskripsi: '', icon: '', warna: '#6366F1', urutan: '0', gambar: null,
    });
    const [preview, setPreview] = useState(null);

    const submit = (e) => {
        e.preventDefault();
        post(route('kategori.store'), { forceFormData: true });
    };
    ...
    return (
        <AuthenticatedLayout>
            ...
            <div className="mb-6">
                ...
                <h1 className="text-2xl font-bold text-[#000000] mt-1">Tambah Kategori</h1>
            </div>
            <div className="mx-auto max-w-lg card">
                <form onSubmit={submit} className="space-y-4">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <div>
                            <label className="mb-1 block text-sm font-medium text-[#52525b]">Warna</label>
                            <input type="color" value={data.warna} onChange={e => setData('warna', e.target.value)}
                                className="h-10 w-full rounded-lg border border-[#e4e4e7] p-1" />
                        </div>
                        <div>
